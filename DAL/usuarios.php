...
function ObtenerUnUsuarios($pCorreo){
    try {
        ...
        if(mysqli_set_charset($oConexion, "utf8")){
            ...
            if(!$result = mysqli_query($oConexion, "SELECT CORREO, CONTRASENA, NOMBRE, APELLIDO_1,APELLIDO_2, RUTA_IMAGEN, ESTADO FROM TAB_USUARIOS where CORREO = '".$pCorreo."'")) die();  
                $retorno = null;
...
function InsertarUsuarios($pCorreo,$pPass,$pNombre,$pApellido1,$pApellido2){
    ...
    try {
        ...
        // formato de datos utf8
        if(mysqli_set_charset($oConexion, "utf8")){
            $stmt = $oConexion->prepare("INSERT INTO TAB_USUARIOS (CORREO, CONTRASENA, NOMBRE, APELLIDO_1,APELLIDO_2) VALUES (?, ?, ?, ?, ?)");
